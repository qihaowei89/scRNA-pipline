...
![图 1 10x Genomics平台](src/10x Genomics平台.png){width=50% }    
...
<center>    
![图 2 Chromium^TM^ Single Cell Gene Expression Solution项目流程图](src/项目流程.png){width=20% }    
...
     
<center>   
![图 3 10x Genomics 标记单细胞文库流程](src/10x Genomics 标记单细胞文库流程.png){width=50%}        
...
![](src/文库构建结果.png){width=60%}    
![](src/VDJ文库构建结果.png){width=60%}   
...
<center>
![图 5 数据基础分析流程图](src/数据基础分析流程图.png){width=60%} 
</center> 
...
![图 6 FASTQ文件格式](src/FASTQ文件格式.png){width=70%}     
...
![](1.Data_assess/GP1-5_S2_L004_R1_001_fastqc/per_base_quality.png){width=35%}![](1.Data_assess/GP1-5_S2_L004_R1_001_fastqc/per_base_sequence_content.png){width=35%}     
...
### 4.3 免疫组库    
  使用Cellranger vdj分析得到单细胞 V(D)J 序列及其注释信息。此部分见 2.Basic_analysis/2.4.BCR。    
  免疫细胞克隆信息如下：   

```{r error=FALSE,echo=F}
library(knitr)
library(kableExtra)
d1 = read.table("2.Basic_analysis/2.4 BCR/clonotypes.csv",sep = ",",header = T,check.names = F)
d1 %>% head ()%>% kbl() %>%
  kable_styling("striped", full_width = T, font_size = 15,
              html_font = "Glyphicons Halflings") %>%
  row_spec(0, bold = TRUE, background = '#E2EDFA',hline_after = T) %>% scroll_box(width = "500px", height = "200px")
...
library(knitr)
library(kableExtra)
d1 = read.table("2.Basic_analysis/2.4 BCR/filtered_contig_annotations.csv",sep = ",",header = T,check.names = F)
d1 %>% head () %>%kbl() %>%
  kable_styling("striped", full_width = T, font_size = 15,
              html_font = "Glyphicons Halflings") %>%
  row_spec(0, bold = TRUE, background = '#E2EDFA',hline_after = T)  %>% scroll_box(width = "500px", height = "200px")
...
### 4.4 Cell Ranger 细胞分群及差异分析    

#### 4.4.1 细胞分群        

  为了深入剖析样品的异质性，Cellranger 基于基因表达水平对细胞进行分群。分群的思路：首先对表达数据 UMI 归一化，然后 PCA 降维分析，选取10个维度采用 Graph-based【1】，k-means【2】（k=2..10）聚类算法进行细胞分群，同时进行t-SNE【3】降维分析用于可视化。此部分分析见3.Cellranger_advanced。   
...
library(knitr)
library(kableExtra)
table1 = read.table(file = "3.Cellranger_advanced/Cluster_specific_genes/graphclust/graphclust.differential_expression.csv",sep = ",",header = T)
