...
# INPUTFASTQ.DIR = "/data/wqihao/Project.sc/cellrangeRun/inputFile/GP"
INPUT.DIR ="/data/wqihao/Project.sc/cellrangeRun/runDir/scRNA_20210511_aggr_GP1and2/" 
INPUT.OUTS.DIR = file.path(INPUT.DIR,"outs")
# sampleName = basename(INPUT.DIR)
subdivide.map = structure(c("GP1","GP2"),names=c(1,2))

OUT.DIR = "/data/wqihao/Project.sc/resultsDir/mergeGP1.GP2_test/"
DIRCREATE(OUT.DIR)
OUT.RESTULE.DIR = file.path(OUT.DIR)
...
COPYFILE(from.name = rawFeatureMatrix.files[1],to.name = OUT.Basic_analysis.DIR[1])
COPYFILE(from.name = rawFeatureMatrix.files[2],to.name = OUT.Basic_analysis.DIR[1])
COPYFILE(from.name = rawFeatureMatrix.files[3],to.name = OUT.Basic_analysis.DIR[1])
DIRCREATE(OUT.Basic_analysis.DIR[2])
COPYFILE(from.name = filteredFeatureMatrix.files[1],to.name = OUT.Basic_analysis.DIR[2])
COPYFILE(from.name = filteredFeatureMatrix.files[2],to.name = OUT.Basic_analysis.DIR[2])
COPYFILE(from.name = filteredFeatureMatrix.files[3],to.name = OUT.Basic_analysis.DIR[2])
DIRCREATE(OUT.Basic_analysis.DIR[3])
COPYFILE(from.name = rawH5,to.name = OUT.Basic_analysis.DIR[3])
COPYFILE(from.name = filteredH5,to.name = file.path(OUT.Basic_analysis.DIR[3]))
DIRCREATE(OUT.Basic_analysis.DIR[4])
COPYFILE(from.name = bcfFiles,to.name = OUT.Basic_analysis.DIR[4])

COPYFILE(from.name = aggregation.csv,to.name =  OUT.TREE.DIR[1])
COPYFILE(from.name = SummaryJson,to.name = OUT.TREE.DIR[1])
COPYFILE(from.name = web_summary.html,to.name = OUT.TREE.DIR[1])
...
COPYFILE(from.name = cloupeFiles,to.name = OUT.cloupeFiles)
COPYFILE(from.name = vloupeFiles,to.name = OUT.vloupeFiles)

# Cluster_specific_genes ------ 未测试
analysisDIR = list.dirs(INPUT.OUTS.DIR,recursive = T,full.names = T) %>% grep(pattern = "analysis",value = T) 
# analysisFiles = GETFILE(input.dir = INPUT.OUTS.DIR,pattern = "analysis")
...
for (n in 1:10) {
  ...
  COPYFILE(from.name = analysis.diffexp[n],to.name = file.path(tmp.DIR,paste0(clusterType,".",basename(analysis.diffexp[n]))))
...
for (n in 1:10) {
  print(n)
  clusterType = basename(dirname(analysis.clusters[n]))
  if(file.exists(file.path(OUT.Cellranger_advanced.DIR[3],paste0(clusterType,".diffexp.volvano.pdf")))){
    next
  }
...
# tsne -----
message("2.Cellranger_advanced: tsne\n")
...
# tsne subdivide ------  
message("2.Cellranger_advanced: tsne subdivide\n")
...
# umap -----
message("2.Cellranger_advanced: umap\n")
...
message("2.Cellranger_advanced: umap subdivide\n")
...
p1 = VlnPlot(seurat_object,features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3,pt.size = 0.1)
...
# 2.pca_analysis ----------
message("3.seurat : 2.pca_analysis\n")

npcs=30
if (!file.exists(file.path(OUT.DIR,"seurat_object.RunPCA.RDS"))) {
  message("3.seurat : 2.pca_analysis RunPCA\n")
  seurat_object <- NormalizeData(seurat_object, normalization.method = "LogNormalize", scale.factor = 10000)
  seurat_object <- FindVariableFeatures(object = seurat_object,selection.method = "vst")
  
  seurat_object <- ScaleData(seurat_object,features = rownames(seurat_object))     
  seurat_object <- RunPCA(object= seurat_object,pc.genes=VariableFeatures(object = seurat_object),npcs = npcs)  
  saveRDS(seurat_object,file = file.path(OUT.DIR,"seurat_object.RunPCA.RDS"))
}else{
  message("3.seurat : skip 2.pca_analysis RunPCA\n")
  seurat_object <- readRDS(file.path(OUT.DIR,"seurat_object.RunPCA.RDS"))
}
...
if(!file.exists(file.path(OUT.DIR,"seurat_object.RunJackStraw.RDS"))){
  message("3.seurat : 2.pca_analysis RunJackStraw\n")
  seurat_object <- JackStraw(seurat_object, num.replicate = 100,dims = npcs)
  seurat_object <- ScoreJackStraw(seurat_object, dims = 1:npcs)
  saveRDS(seurat_object,file = file.path(OUT.DIR,"seurat_object.RunJackStraw.RDS"))
}else{
  message("3.seurat : skip 2.pca_analysis RunJackStraw\n")
  seurat_object <- readRDS(file.path(OUT.DIR,"seurat_object.RunJackStraw.RDS"))
}
...
# 3.Cluster_and_diff ----------
message("3.seurat : 3.Cluster_and_diff\n")
...
if(!file.exists(file.path(OUT.DIR,"seurat_object.RunTSNE.RDS"))){
  message("3.seurat : 3.Cluster_and_diff RunTSNE\n")
  ...
  saveRDS(seurat_object,file = file.path(OUT.DIR,"seurat_object.RunTSNE.RDS"))
}else{
  message("3.seurat : skip 3.Cluster_and_diff RunTSNE\n")
  seurat_object <- readRDS(file.path(OUT.DIR,"seurat_object.RunTSNE.RDS"))  
}
...
group = subdivide 
# Seurat::SplitObject(seurat_object,split.by = "subdivide")
...
if(!file.exists(file.path(OUT.DIR,"seurat_object.markers.RDS"))){
  message("3.seurat : 3.Cluster_and_diff FindAllMarkers\n")
  seurat_object.markers <- FindAllMarkers(seurat_object, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
  saveRDS(seurat_object.markers,file = file.path(OUT.DIR,"seurat_object.markers.RDS"))
}else{
  message("3.seurat : 3.Cluster_and_diff skip FindAllMarkers\n")
  seurat_object.markers = readRDS(file.path(OUT.DIR,"seurat_object.markers.RDS"))
}
...
if(!file.exists(file.path(OUT.DIR,"seurat_object.filted.RDS"))){
  ...
  seurat_object <- FindVariableFeatures(object = seurat_object,selection.method = "vst")
  seurat_object <- ScaleData(seurat_object,features = rownames(seurat_object))     
  seurat_object <- RunPCA(object= seurat_object,pc.genes=VariableFeatures(object = seurat_object),npcs = 50,nfeatures.print = 20,ndims.print = 1:50)  
...
if(!file.exists(file.path(OUT.DIR,"seurat_object.filted.celltype.RDS"))){
  ...
  sc_object <- logNormCounts(sc_object)
...
for (cluster in clusters) {
  ...
  de.genes = graphclust.diffexp[abs(graphclust.diffexp[,fc.cols])>0.1&graphclust.diffexp[,p.cols]<0.05,]
  if(nrow(de.genes)<=3) {
    
    write.table(x = paste0(cluster,"no sig gene"),file = file.path(tmp.DIR,"err.txt"),append = T,quote = F,col.names = F)
  ...
  for (n in c("BP","CC","MF")) {
    ...
    p2 <- dotplot(ego.BP, showCategory=30)
    p3 <- emapplot(ego.BP,layout="kk")
...
# SEURAT
# i = unique(seurat_object$seurat_clusters)[1]
for (i in unique(seurat_object$seurat_clusters)) {
  print(i)
  ...
  cluster.markers = read.table(file.path(OUT.ClusterAndDiff.DIR[1],inputname),sep = "\t",header = T,check.names = F)
  ...
  write.table(de.genes.tmp,file = file.path(tmp.DIR,paste0("Cluster.",i,".gene.txt")),quote = F,sep = "\t",row.names = F,col.names = T)
  # cluster.markers.sig$avg_logFC>1
  
  # head(cluster.markers.sig)
  # head(de.genes.tmp)
